...
class xpwiki_plugin_popular extends xpwiki_plugin {
	...
	// $Id: popular.inc.php,v 1.1 2006/10/13 13:17:49 nao-pon Exp $
	//
	...
	//	global $_popular_plugin_frame, $_popular_plugin_today_frame;
		...
		case 3: if ($array[2]) $today = $this->func->get_date('Y/m/d');
		case 2: $except = $array[1];
		case 1: $max    = $array[0];
		}
	
		$counters = array();
		foreach ($this->func->get_existpages($this->cont['COUNTER_DIR'], '.count') as $file=>$page) {
			if (($except != '' && ereg($except, $page)) ||
			    $page == $this->root->whatsnew || $this->func->check_non_list($page) ||
		...
		if (! empty($counters)) {
			...
			foreach ($counters as $page=>$count) {
				...
				if ($page == $this->root->vars['page']) {
					...
					$pg_passage = $this->func->get_pg_passage($page,FALSE);
					$items .= ' <li><span title="' . $s_page . ' ' . $pg_passage . '">' .
					$s_page . '<span class="counter">(' . $count .
					')</span></span></li>' . "\n";
				} else {
					$items .= ' <li>' . $this->func->make_pagelink($page,
		...
		return sprintf($today ? $this->root->_popular_plugin_today_frame : $this->root->_popular_plugin_frame, count($counters), $items);
	}
}
?>