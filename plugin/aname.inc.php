...
class xpwiki_plugin_aname extends xpwiki_plugin {
	...
	// $Id: aname.inc.php,v 1.3 2007/05/26 01:00:05 nao-pon Exp $
	// Copyright (C)
	...
	function plugin_aname_convert()
	{
		...
		if (func_num_args() < 1)
			return $this->func->plugin_aname_usage($convert);
	...
	//	static $_id = array();
		...
		if (empty($args) || $args[0] == '') return $this->func->plugin_aname_usage($convert);
		...
		if ($body == '') {
			if ($f_noid)  return $this->func->plugin_aname_usage($convert, 'Meaningless(No link-title with \'noid\')');
			if ($f_super) return $this->func->plugin_aname_usage($convert, 'Meaningless(No link-title with \'super\')');
			if ($f_full)  return $this->func->plugin_aname_usage($convert, 'Meaningless(No link-title with \'full\')');
		}
	
		if ($this->cont['PLUGIN_ANAME_ID_MUST_UNIQUE'] && isset($_id[$this->xpwiki->pid][$id]) && ! $f_noid) {
			return $this->func->plugin_aname_usage($convert, 'ID already used: '. $id);
		} else {
			if (strlen($id) > $this->cont['PLUGIN_ANAME_ID_MAX'])
				return $this->func->plugin_aname_usage($convert, 'ID too long');
			if (! preg_match($this->cont['PLUGIN_ANAME_ID_REGEX'], $id))
				return $this->func->plugin_aname_usage($convert, 'Invalid ID string: ' .
				htmlspecialchars($id));
